function[timestore,xstore,ystore,zstore,U_r,adsorption_particle] = fun_SiO2_1000mM(r,phi,D,vp)

global Q kB T mu  height width Length roughness nstatmax dt N c
global N_A e epsilon0 epsilon A zetap zetaw
...
%thickness of depletion zone at the entrance
...
for nstat=1:nstatmax
    ...
    while (n_particle < N)
        ...
        while (adsorption_flag==0)&(exit_flag==0)
            ...
            F_vdw_z = -A/6*r/(dist_z)^2;
            F_vdw_y = -A/6*r/(dist_y)^2;
            
            F_edl_y = C*exp(-dist_y/debye_length);
            F_edl_z = C*exp(-dist_z/debye_length);
            
            % Calcul de la vitesse Vfluid
            Vfluid=0;
            for n=1:5
                betan = (2*n-1)*pi/width;
            ...
            %The particle is reflected on the median plane
            if (z(i)>height/2)
                z(i)=height-z(i);
            end  %the particles are reflected on the median plane h/2
            if (y(i)>width/2)
                y(i)=width-y(i);
            end    %the particles are reflected on the median plane w/2
            
            %The particle irreversibly adsorbs on z=0 (wall) if it approaches at a
            %distance < sigma
            ...
            if (z(i)<r+epsz)
                ...
                %zstore contain coordinates of adsorbed particles
                adsorption_flag=1;
            end
            
            %The particle irreversibly adsorbs on z=0 (wall) if it approaches at a
            %distance < sigma, but we neglect particles adsorbed on sidewalls
            
            if (y(i)<r+epsy)
                adsorption_flag=1;
            end
            
            %The particle exits the channel without sticking
            if (x(i)>Length)
                exit_flag=1;
