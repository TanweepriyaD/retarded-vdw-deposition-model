function[timestore,xstore,ystore,zstore,U_r,adsorption_particle] = fun_airBalloon_2nmPS_250mM(r,phi,D,vp)
    ...
    while (n_particle < N)
        ...
        while (adsorption_flag==0)&(exit_flag==0)
            ...
            elseif (0.5e-9<dist_z)&&(dist_z<=0.7e-9)
                F_vdw_z = -2.232e-27*exp(-2.598*log(dist_z))*r;
            elseif (0.7e-9<dist_z)&&(dist_z<=25.1e-9)
                F_vdw_z = (interp1(L,vdwr,dist_z))*r;
            elseif (25e-9<dist_z)&&(dist_z<=40e-9)
                F_vdw_z = 5.479e-26*exp(-2.514*log(dist_z))*r;
            elseif (40e-9<dist_z)&&(dist_z<=50e-9)
                F_vdw_z = 5.783e-27*exp(-2.646*log(dist_z))*r;
            elseif (50e-9<dist_z)&&(dist_z<=70e-9)
                F_vdw_z = 1.879e-27*exp(-2.713*log(dist_z))*r;
            elseif (70e-9<dist_z)&&(dist_z<=100e-9)
                F_vdw_z = 7.413e-28*exp(-2.769*log(dist_z))*r;
            elseif (100e-9<dist_z)&&(dist_z<=155e-9)
                F_vdw_z = 3.482e-28*exp(-2.816*log(dist_z))*r;
            elseif (155e-9<dist_z)&&(dist_z<=251e-9)
                F_vdw_z = 1.252e-28*exp(-2.881*log(dist_z))*r;
            elseif (251e-9<dist_z)&&(dist_z<=400e-9)
                F_vdw_z = 2.025e-29*exp(-3.001*log(dist_z))*r;
            elseif (400e-9<dist_z)
                F_vdw_z = 1.858e-30*exp(-3.163*log(dist_z))*r;
            end
            
            if (dist_y<=0.5e-9)
                F_vdw_y = (interp1(L,vdwr,dist_y))*r;
            elseif (0.5e-9<dist_y)&&(dist_y<=0.7e-9)
                F_vdw_y = -2.232e-27*exp(-2.598*log(dist_y))*r;
            elseif (0.7e-9<dist_y)&&(dist_y<=25.1e-9)
                F_vdw_y = (interp1(L,vdwr,dist_y))*r;
            elseif (25e-9<dist_y)&&(dist_y<=40e-9)
                F_vdw_y = 5.479e-26*exp(-2.514*log(dist_y))*r;
            elseif (40e-9<dist_y)&&(dist_y<=50e-9)
                F_vdw_y = 5.783e-27*exp(-2.646*log(dist_y))*r;
            elseif (50e-9<dist_y)&&(dist_y<=70e-9)
                F_vdw_y = 1.879e-27*exp(-2.713*log(dist_y))*r;
            elseif (70e-9<dist_y)&&(dist_y<=100e-9)
                F_vdw_y = 7.413e-28*exp(-2.769*log(dist_y))*r;
            elseif (100e-9<dist_y)&&(dist_y<=155e-9)
                F_vdw_y = 3.482e-28*exp(-2.816*log(dist_y))*r;
            elseif (155e-9<dist_y)&&(dist_y<=251e-9)
                F_vdw_y = 1.252e-28*exp(-2.881*log(dist_y))*r;
            elseif (251e-9<dist_y)&&(dist_y<=400e-9)
                F_vdw_y = 2.025e-29*exp(-3.001*log(dist_y))*r;
            elseif (400e-9<dist_y)
                F_vdw_y = 1.858e-30*exp(-3.163*log(dist_y))*r;
            end
